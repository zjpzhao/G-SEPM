...
void print(DATA_TYPE* C_outputFromGpu){
	...
	for (i = 0 ; i < NI ; ++i)
	{
		for(j = 0 ; j < NJ ; ++j)
		{
		sprintf(str,"%f\t",C_outputFromGpu[i*NJ + j]);
		fputs(str,fp);
		}
		sprintf(str,"\n");
		fputs(str,fp);
...
__global__ void gemm_kernel(DATA_TYPE *a, DATA_TYPE *b, DATA_TYPE *c, DT *f)
{
...
void gemmCuda(DATA_TYPE* A, DATA_TYPE* B, DATA_TYPE* C, DATA_TYPE* C_outputFromGpu, DT* f)
{
	...
	gemm_kernel<<< grid, block >>>(A_gpu, B_gpu, C_gpu, F_gpu);
...
//	double t_start, t_end;
	...
	gemmCuda(A, B, C, C_outputFromGpu, f);
