...
void print(DATA_TYPE* x1_outputFromGpu,DATA_TYPE* x2_outputFromGpu){
	FILE* fp;
	fp=fopen("out.txt","w");
	char str[STR_SIZE];
...
__global__ void mvt_kernel1(DATA_TYPE *a, DATA_TYPE *x1, DATA_TYPE *y_1, DT* f)
{
...
__global__ void mvt_kernel2(DATA_TYPE *a, DATA_TYPE *x2, DATA_TYPE *y_2, DT* f)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
...
void mvtCuda(DATA_TYPE* a, DATA_TYPE* x1, DATA_TYPE* x2, DATA_TYPE* y_1, DATA_TYPE* y_2, 
			DATA_TYPE* x1_outputFromGpu, DATA_TYPE* x2_outputFromGpu, DT* f)
{
	...
	t_start = rtclock();
	mvt_kernel1<<<grid,block>>>(a_gpu,x1_gpu,y_1_gpu, F_gpu);
	mvt_kernel2<<<grid,block>>>(a_gpu,x2_gpu,y_2_gpu, F_gpu);
...
int main()
{
	...
	mvtCuda(a, x1, x2, y_1, y_2, x1_outputFromGpu, x2_outputFromGpu, f);
	print(x1_outputFromGpu, x2_outputFromGpu);
	t_start = rtclock();
