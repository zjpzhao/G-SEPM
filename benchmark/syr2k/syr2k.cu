...
void print(DATA_TYPE* C_outputFromGpu){
	...
	for (i = 0 ; i < N ; ++i)
	{
		for (j = 0 ; j < N ; ++j)
		{
			sprintf(str,"%f\t",C_outputFromGpu[i*N + j]);
			fputs(str,fp);
		}
		sprintf(str,"\n");
		fputs(str,fp);
...
__global__ void syr2k_kernel(DATA_TYPE *a, DATA_TYPE *b, DATA_TYPE *c, DT* f)
{
...
void syr2kCuda(DATA_TYPE* A, DATA_TYPE* B, DATA_TYPE* C, DATA_TYPE* C_outputFromGpu, DT* f)
{
	...
	t_start = rtclock();
	syr2k_kernel<<<grid,block>>>(A_gpu,B_gpu,C_gpu, F_gpu);
...
//	double t_start, t_end;
	...
	GPU_argv_init();
	syr2kCuda(A, B, C, C_outputFromGpu, f);
