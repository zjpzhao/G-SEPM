...
void print(float *ary, int ary_size);
 ...
 **-------------------------------------------------------
 */
__global__ void Fan1(float *m_cuda, float *a_cuda, int Size, int t,DT* f)
{   
...
__global__ void Fan2(float *m_cuda, float *a_cuda, float *b_cuda,int Size, int j1, int t,DT* f)
{
	if(threadIdx.x + blockIdx.x * blockDim.x >= Size-1-t) return;
...
void ForwardSub(DT* f)
{
    ...
    gettimeofday(&time_start, NULL);
	for (t=0; t<(Size-1); t++) {
		Fan1<<<dimGrid,dimBlock>>>(m_cuda,a_cuda,Size,t,F_gpu);
		cudaThreadSynchronize();
		Fan2<<<dimGridXY,dimBlockXY>>>(m_cuda,a_cuda,b_cuda,Size,Size-t,t,F_gpu);
		cudaThreadSynchronize();
		checkCUDAError("Fan2");
	}
	// end timing kernels
